...
camera_params_path = os.path.join(acl3d_root, "extra", "camera_parameters.npy")
...
for subject_idx, subject in enumerate(retval['subject_names']):
    for camera_idx, camera in enumerate(retval['camera_names']):
        assert len(cameras_params[subject][action]) == 4
        camera_params = cameras_params[subject][action]['cam%d' % (camera_idx+1)]
        camera_retval = retval['cameras'][subject_idx][camera_idx]
...
# bboxes_path = sys.arv[3]
bboxes_path = os.path.join(acl3d_root, "extra", "bboxes-acl3d-GT.npy")
...
#            poses_world = np.array(poses_file['3D_positions']).T.reshape(-1, 32, 3)[frame_idxs][:, valid_joints]
        ...
        # setting "poses_world" to be zeros because we don't have poses.
        poses_world = np.ones((len(frame_idxs), 17, 3))
...
print("Total frames in ACL3D:", len(retval['table']))
