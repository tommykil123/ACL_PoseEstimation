...
from mvn.datasets import acl3d
...
def setup_acl3d_dataloaders(config, is_train, distributed_train):
    train_dataloader = None
    train_sampler = None
    if is_train:
        # train
        train_dataset = acl3d.acl3dMultiViewDataset(
            acl3d_root=config.dataset.train.acl3d_root,
    ...
    # val
    val_dataset = acl3d.acl3dMultiViewDataset(
        acl3d_root=config.dataset.val.acl3d_root,
...
def setup_dataloaders(config, is_train=True, distributed_train=False):
    if config.dataset.kind == 'acl3d':
        train_dataloader, val_dataloader, train_sampler = setup_acl3d_dataloaders(config, is_train, distributed_train)
    else:
...
def setup_experiment(config, model_name, is_train=True):
    ...
    experiment_name = '{}_{}'.format(experiment_title, datetime.now().strftime("%d_%m_%Y-%H_%M_%S"))
                ...
                if master:
                    if n_iters_total % config.vis_freq == 0:# or total_l2.item() > 500.0:
                        ...
                        for batch_i in range(min(batch_size, config.vis_n_elements)):
                            ...
                            writer.add_image(f"{name}/keypoints_vis/{n_iters_total}/{batch_i}", keypoints_vis.transpose(2, 0, 1), global_step=n_iters_total)
                            ...
                            writer.add_image(f"{name}/heatmaps/{n_iters_total}/{batch_i}", heatmaps_vis.transpose(2, 0, 1), global_step=n_iters_total)
                            
                            #patrick trying to add 3D skeleton in
#                            fig = plt.figure()
#                            ax = fig.add_subplot(111, projection='3d')
#                            pose_vis = vis.draw_3d_pose(to_numpy(keypoints_3d_pred[0, :]), ax, kind='coco')
#                            writer.add_image(f"{name}/heatmaps/{n_iters_total}/{batch_i}",  pose_vis.transpose(2, 0, 1), global_step=n_iters_total)
                            ...
                            if model_type == "vol":
                                ...
                                writer.add_image(f"{name}/volumes/{n_iters_total}/{batch_i}", volumes_vis.transpose(2, 0, 1), global_step=n_iters_total)
...
def main(args):
    ...
    # datasets
    print("Loading data...")
    train_dataloader, val_dataloader, train_sampler = setup_dataloaders(config, is_train=not args.eval, distributed_train=is_distributed)
